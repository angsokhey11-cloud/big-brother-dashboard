/* BIG BROTHER — Sales Support Your Receivable menu V1 */
(function(){
'use strict';

const ROUTE='sales-support-your-receivable';
const URL='https://angsokhey11-cloud.github.io/big-brother-ar/your-receivable.html?embed=1&view=your&v=20260914-1';
...
function ensureMenu(){
  ...
  let btn=document.getElementById('navSalesSupportYourReceivable');
  if(!btn){
    btn=document.createElement('button');
    btn.type='button';
    btn.id='navSalesSupportYourReceivable';
    btn.textContent='💳 Your Receivable';
    btn.addEventListener('click',()=>open(true));
    const invoices=document.getElementById('navSalesSupportYourInvoices');
    const collection=document.getElementById('navSalesSupportYourCollection');
    if(invoices?.parentNode===submenu)invoices.insertAdjacentElement('afterend',btn);
    else if(collection?.parentNode===submenu)collection.insertAdjacentElement('afterend',btn);
...
function markActive(){
  ...
  document.getElementById('navSalesSupportYourReceivable')?.classList.add('active');
  const submenu=document.getElementById('salesSupportSubmenu');
...
function deny(){
  try{
    document.getElementById('bbYourReceivableToast')?.remove();
    const el=document.createElement('div');
    el.id='bbYourReceivableToast';
    el.textContent='Access denied for Your Receivable.';
...
window.BBYourReceivable={open,ensureMenu,canView};
