/* BIG BROTHER — Sales Support Your Collection menu V1 */
(function(){
'use strict';

const ROUTE='sales-support-your-collection';
const URL='https://angsokhey11-cloud.github.io/big-brother-daily-cash-collection/your-collection.html?embed=1&v=20260914-1';
...
function ensureMenu(){
  ...
  let btn=document.getElementById('navSalesSupportYourCollection');
  if(!btn){
    btn=document.createElement('button');
    btn.type='button';
    btn.id='navSalesSupportYourCollection';
    btn.textContent='💵 Your Collection';
...
function markActive(){
  ...
  document.getElementById('navSalesSupportYourCollection')?.classList.add('active');
  const submenu=document.getElementById('salesSupportSubmenu');
...
function deny(){
  try{
    const old=document.getElementById('bbYourCollectionToast');
    old?.remove();
    const el=document.createElement('div');
    el.id='bbYourCollectionToast';
    el.textContent='Access denied for Your Collection.';
...
window.BBYourCollection={open,ensureMenu,canView};
