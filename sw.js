...
const CACHE='bb-mobile-shell-v52';
...
function injectRouter(html){
  let out=html;
  if(!out.includes('data-bb-router-v4')){const tag='<script data-bb-router-v4 src="mobile-main-menu-v3.js?v=20260921-reportcontrast1"></script>';out=out.includes('</body>')?out.replace('</body>',tag+'\n</body>'):out+tag}
  if(!out.includes('data-bb-user-overview-v4')){const tag='<script data-bb-user-overview-v4 src="mobile-user-overview.js?v=20260920-liveoverview2"></script>';out=out.includes('</body>')?out.replace('</body>',tag+'\n</body>'):out+tag}
